...
<a href="https://www.toniagara.com">www.toniagara.com</a></p>
		...
		$email_id = str_replace(array("<",">"),"",$result->id);
		if($email_id) {
			saveLog($conn, $_GET['id'], $email_id, 'Abandone from backend');
		}
		//sendMail( array('ashutosh2801@gmail.com', SITENAME), "Final Followup: $subject", $HTML);
            ...
            <div class="agile-tables">
              <div class="w3l-table-info agile_info_shadow">
                ...
                <table id="table"  class="table">
                  <thead>
                  	<tr>
                    	<th colspan="10" style="background:#fff"><button onClick="return confirm('Do you want to send to TRASH?')" type="submit" class="btn btn-danger" name="trash"><i class="fa fa-trash"></i> Trash All</button></th>
                    </tr>
                    <tr>
                      <th width="20">#</th>
                  ...
                  <?php
                    ...
                    <tr bgcolor="#d4e8d4">
                      ...
                      <td><a href="<?php echo $row->email; ?>"><?php echo $row->email; ?></a> <?php echo is_delivered($conn, $row->id);  ?> <a href="#" onclick="return myFunction('<?php echo 'click'.$row->id; ?>')">Click me</a></td>
                      <td><?php echo $row->coupon; ?> <input type="hidden" id="<?php echo 'click'.$row->id; ?>" value="https://www.toniagara.com/checkout?token=<?php echo $row->sessionId; ?>" /></td>
                      <td>
                      ...
                      <td>
                      	<a href="ajax/resume.php?id=<?php echo $row->id; ?>" class="badge badge-success resume"><i class="fa fa-copy"></i> Resume</a>
                      	<?php /*?><a href="?id=<?php echo $row->id; ?>&act=resend" class="badge badge-success" onClick="return confirm('Do you want to RESEND?')"><i class="fa fa-envelope"></i> Resend</a><?php */?>
                    ...
                    <tr bgcolor="#d4e8d4">
                    <td colspan="10" style="padding: 0px 10px 0px 0px">
                    ...
                    <tbody>
                    <?php
                    $sql3 = "SELECT c.id, c.status, c.order_id, c.created_date, cd.*, t.ID, t.title, t.sub_title, t.slug, t.tour_thumb, t.price_type FROM `".PREFIX."customers` c, `".PREFIX."customer_detail` cd, ".PREFIX."tours t WHERE c.id=".$row->id." AND  c.id=cd.user_id AND cd.tour_id=t.ID AND c.status=0 ORDER BY cd.tour_date DESC";
                  ...
                  <?php  
									...
									?>
                  <tr><td colspan='10' class="text-center"><?php echo paginattion($page, ($result->num_rows/$limit) ,'/admin/customers_abandone.php?q='); ?></td></tr>
                  </tbody>
                </table>
                </form>
          		</div>
...
$(document).ready(function() {
	...
	$('.resume').click(function(){
		...
		$.ajax({url:href, data:'', type:'POST', success:function(msg){
