...
if(!empty($orderId))
{
	...
	if($query->num_rows > 0 ) 
	{
		...
		try {
			...
			$charge = \Stripe\Charge::retrieve($model3->auth_id);
			$charge->capture();
			...
			if($chargeJson['id'] && $chargeJson['status']=="succeeded" && $chargeJson['amount_refunded'] == 0 && empty($chargeJson['failure_code']) && $chargeJson['paid'] == 1 && $chargeJson['captured'] == 1) {
				$sql_2 = "UPDATE `tour_secure_payment` SET `transaction_id`='".$chargeJson['balance_transaction']."', `status`='2' WHERE id=".$model3->id.";";
				$conn->query($sql_2);
			$statusMsg = '<p class="alert alert-success">Payment accepted.</p>';
			}
			else {
...
<html lang="en">
<head>
<title>Accept - Secure Payments :: <?php echo SITENAME; ?></title>
