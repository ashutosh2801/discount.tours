...
$url_string = '';
$sql = "SELECT * FROM tour_users WHERE ID='$agent_id'";
...
if(!empty($_GET['id']))
{
	...
	if ( $tour_detail->ID ) {
		...
		header('Location: '.APPROOT.'agents/error?url='.$_SESSION['page_url']);
		exit;
	}
} 
else {
	header('Location: '.APPROOT.'agents/');
	exit;
}
...
if(!empty($_POST['id']))
{
	...
	if(	$flag == 0 )
	{
		...
		if ($user_id) 
		{
			...
			$conn->query($sql2);
			header('Location: '.APPROOT.'agents/add_new_booking_checkout.php');
			exit;
		}
	}
}
