...
if(isset($_POST['cust_order']) && isset($_POST['completed'])) {
	foreach($_POST['cust_order'] as $id) {
		$sql = "UPDATE `".PREFIX."customers` SET `status`=4 WHERE id=".$id." AND agent_id=$agent_id;";
		$conn->query($sql);
...
if(isset($_GET['id']) && ($_GET['act']=='complete')) {
	$sql = "UPDATE `".PREFIX."customers` SET `status`=4 WHERE id=".$_GET['id']." AND agent_id=$agent_id;";
	$conn->query($sql);
...
if( isset($_POST['export']) ) {
	...
	$condition = "c.id=cd.user_id AND c.status=2 AND cd.tour_date>='".date('Y-m-d')."' AND agent_id=$agent_id";
	if(!empty($_POST['q'])) {
                ...
                <table id="table"  class="table">
                  ...
                  <?php
                    ...
                    <tr bgcolor="#d4e8d4">
                      ...
                      <td><?php echo ($row->status && $row->type=='Card') ? '<a class="badge badge-success">Paid</a>':'<a class="badge badge-warning">Balance</a>'; ?></td>
                      <td>
                      	...
                      	<a href="?id=<?php echo $row->id; ?>&act=complete" class="badge badge-success" onClick="return confirm('Do you want to create Mark as COMPLETED?')"><i class="fa fa-check"></i> Mark as complete</a><?php */?>
                      </td>
                    </tr>
                    ...
                    <thead>
                      ...
                      <th width="100" style="background:#4CAF50;text-align:right">Discount</th>
                    ...
                    <tbody>
                    <?php
                    $sql3 = "SELECT c.id, c.status, c.order_id, c.created_date, cd.*, t.ID, t.title, t.sub_title, t.slug, t.tour_thumb, t.price_type FROM `".PREFIX."customers` c, `".PREFIX."customer_detail` cd, ".PREFIX."tours t WHERE c.id=".$row->id." AND  c.id=cd.user_id AND cd.tour_id=t.ID AND c.status=2 AND agent_id=$agent_id ORDER BY cd.tour_date ASC";
