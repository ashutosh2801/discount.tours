...
if(!empty($_POST['type']))
{
    ...
    $card_exp_year 	= $_POST['exp_year'];
		...
		$sql_2 = "UPDATE `tour_customers` SET `status`='1', `type`='".$_POST['type']."', `note`='".$_POST['note']."' WHERE id=".$model3->id.";";
...
<div class="wthree_agile_admin_info">
    ...
    <?php
                ...
                <div class="col-md-6 button_set_one agile_info_shadow graph-form">
                  <div class="grid-1">
                    ...
                    <div class="form-group">
                    	<input type="radio" value="Card" name="type" id="card"> Card
                    	<?php /*?><input type="radio" value="Cash" name="type" id="cash" <?php if($_GET['type']=='Cash') echo 'checked'; ?>> Cash
                    	<input type="radio" value="Check" name="type" id="check" <?php if($_GET['type']=='Check') echo 'checked'; ?>> Check<?php */?>
                    	<input type="radio" value="Pay Later" name="type" id="pay_later" <?php if($_GET['type']=='Pay Later') echo 'checked'; ?>> Pay Later
                    </div>
                    
                    <div class="form-group">
                        <input type="hidden" class="form-control1" name="note" placeholder="Note" />
                    </div>
        ...
        <tr class="discount" style="color:#F00">
          <td>Discount <a href="add_new_booking_checkout2.php"><i class="fa fa-times-circle"></i> Remove</a></td>
          <td align="right">CA$<?=$whole; ?> <sup><?=$decimal; ?></sup></td>
        </tr>
        <?php } else if(($not==1)) {  ?>
        <tr class="discount" style="color:#F00">
        ...
        <?php } else { $total = ($subtotal + $tax); } ?>
                    ...
                    <div class="form-group text-center">
                        ...
                        <button style="padding:10px; font-size:30px;" class="btn btn-success" id="payBtn" type="submit"><i class="fa fa-cart-arrow-down"></i> Pay CA$<?php echo number_format($total,2); ?></button>
                      </div>
...
$(document).ready(function() {
   ...
   $(this).data('oldValue', $(this).val());
	...
	$('#card').click(function(){ 
		window.location.href = 'add_new_booking_checkout.php?promo=<?=$_GET['promo'];?>';
	});
