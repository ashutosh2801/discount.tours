...
if(isset($_POST['cust_order']) && isset($_POST['restore'])) {
	foreach($_POST['cust_order'] as $id) {
		$sql = "UPDATE `".PREFIX."customers` SET `status`=1 WHERE id=".$id." AND agent_id=$agent_id;";
		$conn->query($sql);
	}
...
if(isset($_POST['cust_order']) && isset($_POST['delete'])) {
	foreach($_POST['cust_order'] as $id) {
		$sql = "DELETE FROM `".PREFIX."customers` WHERE id=$id AND agent_id=$agent_id;";
		$conn->query($sql);
...
if(isset($_GET['id']) && ($_GET['act']=='restore')) {
	$sql = "UPDATE `".PREFIX."customers` SET `status`=1 WHERE id=".$_GET['id']." AND agent_id=$agent_id;";
	$conn->query($sql);
          ...
          <div class="w3l_agileits_breadcrumbs">
            <div class="w3l_agileits_breadcrumbs_inner">
              <ul>
                ...
                <li><a href="customers_unconfirm.php">Trash Booking</a><span>«</span></li>
                <li>List</li>
              </ul>
            </div>
          </div>
            ...
            <div class="agile-tables">
              <div class="w3l-table-info agile_info_shadow">
                ...
                <table id="table"  class="table">
                  ...
                  <tbody>
                  <?php
									$condition = "c.id=cd.user_id AND c.status=3 AND agent_id=$agent_id";
									if(!empty($_GET['q'])) {
                    ...
                    <tbody>
                    <?php
                    $sql3 = "SELECT c.id, c.status, c.order_id, c.created_date, cd.*, t.ID, t.title, t.sub_title, t.slug, t.tour_thumb, t.price_type FROM `".PREFIX."customers` c, `".PREFIX."customer_detail` cd, ".PREFIX."tours t WHERE c.id=".$row->id." AND  c.id=cd.user_id AND cd.tour_id=t.ID AND c.status=3 AND agent_id=$agent_id ORDER BY cd.tour_date DESC";
